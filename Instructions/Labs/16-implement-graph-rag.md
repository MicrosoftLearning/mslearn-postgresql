...
1. In your Azure Cloud Shell, use the `COPY` command to load data from CSV files into each table you previously created. Run the following command to populate the `company_policies` table:
    ...
    ```sql
    \COPY company_policies (title, department, policy_text, category) FROM 'mslearn-postgresql/Allfiles/Labs/Shared/company_policies_with_categories.csv' WITH (FORMAT csv, HEADER)
    ```
...
1. Backfill embeddings for existing rows.
    ...
    Once all 108 rows are backfilled, exit *psql* by typing `\q`.
...
## Enable the Apache AGE Extension on the Azure portal
